...
import dhimmis.reporting.report_database
...
def create_client(testusers, port, user_db_content, readonly=True):
    ...
    os.environ['DHIMMIS_USER_FILE'] = users_fname
    reports_fd, reports_fname = tempfile.mkstemp('', 'reports_', dir='/dev/shm')
    os.environ['DHIMMIS_REPORT_FILE'] = reports_fname

    os.environ['DHIMMIS_ENVIRONMENT'] = 'PYTEST'
    ...
    cur.execute(dhimmis.reporting.report_database._database_schema)

    conn.commit()
    conn.close()

    app = dhimmis.create_app()
