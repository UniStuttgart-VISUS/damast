...
from database.testdata import external_database_table

@pytest.fixture
def url():
    return F'/rest/uri/external-database-list'
...
def test_get(client_ro, minimal_testuser, ro_headers, url):
    ...
    if 'user' in user.roles and 'readdb' in user.roles:
        ...
        assert len(data) == len(external_database_table)
        assert all(map(lambda x: any(map(lambda y: x['id'] == y.id, external_database_table)), data))
        assert all(map(lambda x: any(map(lambda y: x.id == y['id'], data)), external_database_table))
