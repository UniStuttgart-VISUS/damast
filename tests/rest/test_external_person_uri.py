...
from database.testdata import uri_namespace_table, external_database_table, external_person_uri_table

Get = namedtuple('Get', ['id', 'exists', 'datum'])
_get_targets = []
for t in external_person_uri_table:
...
def get_url(get_target):
    return F'/rest/uri/external-person-uri/{get_target.id}'
...
def test_get(client_ro, minimal_testuser, ro_headers, get_target, get_url):
    ...
    if 'user' in user.roles and 'readdb' in user.roles:
        if get_target.exists:
            ...
            assert data['id'] == d.id
            assert data['person_id'] == d.person_id
...
_put_targets = [
        Put(dict(person_id=8, uri_namespace_id=1, uri_fragment='1234'), True, True, 201),
        Put(dict(person_id=2, uri_namespace_id=3, uri_fragment='1234', comment=None), True, True, 201),
        Put(dict(person_id=4, uri_namespace_id=4, uri_fragment='https://www.uris.org/uri-13245', comment='foo'), True, True, 201),

        Put(dict(person_id=16, uri_fragment='1234'), True, False, 400),   # no URI namespace ID
        Put(dict(person_id='string', uri_namespace_id=2, uri_fragment='1234'), True, False, 422),   # person_id is string
        Put(dict(uri_namespace_id=2, uri_fragment='124'), True, False, 400),   # no person_id
        Put(dict(uri_namespace_id=2, person_id=1), True, False, 400),   # no uri_fragment
        Put(dict(person_id=5, uri_namespace_id=3, uri_fragment='1234', comment=None, garbage_content='foo'), True, False, 400),  # invalid data key passed
        Put(dict(person_id=2016, uri_namespace_id=1, uri_fragment='1234'), True, False, 409),   # person does not exist
        Put(dict(person_id=1, uri_namespace_id=7, uri_fragment='1234'), True, False, 409),   # URI namespace does not exist
...
@pytest.fixture
def put_url():
    return '/rest/uri/external-person-uri/0'
...
def test_put(client, minimal_testuser, headers, cursor, put_target, put_url):
    ...
    old_count = cursor.one('SELECT count(*) FROM external_person_uri;')
    rv = client.put(put_url, headers=hdr, data=data)
    new_count = cursor.one('SELECT count(*) FROM external_person_uri;')
    ...
    if 'user' in user.roles and 'readdb' in user.roles and 'writedb' in user.roles:
        ...
        if put_target.success:
            ...
            assert 'external_person_uri_id' in ret

            assert new_count == old_count + 1

            new_tuple = cursor.one('SELECT * FROM external_person_uri WHERE id = %s;', (ret['external_person_uri_id'],))._asdict()
...
def test_delete(client, minimal_testuser, headers, cursor, delete_target, delete_url):
    ...
    if 'user' in user.roles and 'readdb' in user.roles and 'writedb' in user.roles:
        if delete_target.exists:
            ...
            assert 'external_person_uri' in ret['deleted']
            assert ret['deleted']['external_person_uri'] == delete_target.id

            assert new_count == old_count - 1
            assert 0 == cursor.one('SELECT COUNT(*) FROM external_person_uri WHERE id = %s;', (delete_target.id,))
...
_patch_data = [
    Patch(dict(person_id=7), True, True, 205),
    Patch(dict(person_id=4, uri_fragment='12'), True, True, 205),
    ...
    Patch(dict(person_id=23456, comment='hey'), True, False, 409),  # person_id does not exist
...
def test_patch(client, minimal_testuser, headers, cursor, patch_target, patch_url, patch_data):
    ...
    if patch_target.exists:
        old_data = cursor.one('SELECT * FROM external_person_uri WHERE id = %s;', (patch_target.id,))._asdict()
    else:
        cursor.execute('SELECT * FROM external_person_uri;')
        old_data = {
            x._asdict()['id']: x._asdict() for x in cursor.fetchall()
                }
    rv = client.patch(patch_url, headers=hdr, data=data)
    if patch_target.exists:
        new_data = cursor.one('SELECT * FROM external_person_uri WHERE id = %s;', (patch_target.id,))._asdict()
    else:
        cursor.execute('SELECT * FROM external_person_uri;')
        new_data = {
            x._asdict()['id']: x._asdict() for x in cursor.fetchall()
