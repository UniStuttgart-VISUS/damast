...
@pytest.fixture
def url(target):
    return F'/rest/place-instance/{target.id}'


def test_get(client_ro, minimal_testuser, ro_headers, target, url):
    '''test GET /rest/place-instance/<id>'''
...
_put_data = [
        PutData(dict(place_id=3), True, True, 201),
        PutData(dict(place_id=12, confidence='contested'), True, True, 201),
        PutData(dict(place_id=2, comment='Test comment'), True, True, 201),
        PutData(dict(place_id=2, comment='argh', confidence='certain', annotation_id=40), True, True, 201),

        PutData(dict(comment='test'), True, False, 422),  # no place_id
        PutData(dict(illegal_key=12, place_id=4), True, False, 422),  # illegal field
        PutData(dict(), True, False, 422),  # empty
        PutData(dict(place_id=1, confidence='maybe'), True, False, 422),  # confidence value invalid

        PutData(dict(place_id=91223, comment=None, annotation_id=37), True, False, 409),  # FK place_id invalid
        PutData(dict(place_id=18, comment=None, annotation_id=3790), True, False, 409),  # FK annotation_id invalid
        PutData(dict(place_id=12, annotation_id=4), True, False, 409),  # annotation_id already used
...
def test_put(client, minimal_testuser, cursor, headers, put_data, put_url):
    ...
    num_instances = cursor.one('select count(*) from place_instance;')
    ...
    if 'user' in user.roles and 'readdb' in user.roles and 'writedb' in user.roles:
        ...
        if put_data.success:
            assert num_instances == len(place_instance_table) + 1

            ret = json.loads(rv.data)
            assert 'place_instance_id' in ret
            assert ret['place_instance_id'] == num_instances
            pass

        else:
            assert num_instances == len(place_instance_table)
...
_patch_targets = [
        PatchTarget(1, dict(place_id=12), True, True, 205),
        PatchTarget(2, dict(comment='new'), True, True, 205),
        PatchTarget(3, dict(confidence='certain', annotation_id=38), True, True, 205),
        PatchTarget(8, dict(annotation_id=12), True, True, 205),  # annotation 12 already on 8
        PatchTarget(16, dict(place_id=4, comment=None, confidence='false'), True, True, 205),

        PatchTarget(5, dict(place_id=None), True, False, 409),
        PatchTarget(6, dict(place_id=120), True, False, 409),
...
def patch_url(patch_target):
    return F'/rest/place-instance/{patch_target.id}'


def test_patch(client, minimal_testuser, cursor, headers, patch_target, patch_url):
    '''test PATCH /rest/place-instance/<id>'''
    user, _ = minimal_testuser
    ...
    query = 'select * from place_instance order by id asc;'
    ...
    if 'user' in user.roles and 'readdb' in user.roles and 'writedb' in user.roles:
        ...
        if patch_target.success:
            ...
            d = cursor.one('select * from place_instance where id = %s;', (patch_target.id,))._asdict()
...
def test_delete(client, minimal_testuser, cursor, headers, target, url):
    '''test DELETE /rest/place-instance/id'''
    user, _ = minimal_testuser
    ...
    if 'user' in user.roles and 'readdb' in user.roles and 'writedb' in user.roles:
        if target.exists:
            evcount = cursor.one('select count(*) from evidence where place_instance_id = %s;', (target.id,))
            if evcount > 0:
                ...
                assert old != new
                assert 0 == cursor.one('select count(*) from place_instance where id = %s;', (target.id,))
                assert len(place_instance_table) - 1 == cursor.one('select count(*) from place_instance;')

                d = json.loads(rv.data)
                assert d['place_instance_id'] == target.id
