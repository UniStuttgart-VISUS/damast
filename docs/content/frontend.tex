...
\begin{description}
  ...
  \item[\texttt{DHIMMIS\_REPORT\_EVICTION\_MAXSIZE}:]
    ...
    If so, it will evict the least-accessed reports until that sum is less than 80\,\% of that size\footnote{
      This hysteresis ensures that the eviction does not need to happen every time the server checks for size.
    }.
